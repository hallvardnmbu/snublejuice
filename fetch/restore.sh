...
if [ -n "$1" ]; then
    ...
    # /home/snublejuice/.local/bin/uv run backups/operations.py restore --date "$1" || abort "Something went wrong!"
else
    log "Running backup without input."
    # /home/snublejuice/.local/bin/uv run backups/operations.py restore || abort "Something went wrong!"
fi
